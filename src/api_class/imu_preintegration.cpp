...
namespace PARTICLE_FILTER_3D{
    ...
    bool IMUPreintegration::getPreintegratedOdom(const sensor_msgs::Imu& imu_data, nav_msgs::Odometry& imu_odom){
    ...
    void IMUPreintegration::imuCallback(const sensor_msgs::ImuConstPtr& imu){
        ...
        bool isok = getPreintegratedOdom(*imu, imu_odom);
        if(isok){
            pub_imu_odom_.publish(imu_odom);
        }
    }

    void IMUPreintegration::optimWithPose(const Eigen::Matrix4d& robot_pose, double currentCorrectionTime){
        unique_lock<mutex> lock(imu_mtx_);        
        if(imuQueOpt_.empty()){
            return;
        }
        gtsam::Pose3 robotPose(robot_pose);
